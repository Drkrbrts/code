...
function NavBar(props) {
  ...
  // console.log("NavBar firing", props.routes);

  useEffect(() => {
    // console.log("useEffect firing");
    if (routes.length > 0) {
      const mappedRoutes = routes.map(mapNavLinks);
      console.log("if firing", mappedRoutes);
      setNavLinks(mappedRoutes);
    }
  }, [routes]);

  const mapNavLinks = (route) => {
    // console.log("mapNavLinks firing", route);
    switch (route.path) {
      case "/":
        return (
          <NavLink to="/">
