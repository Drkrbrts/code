...
function SingleFriend(props) {
    ...
    const onClickDeleteFriend = function (e) {
        props.delete(friend.id); // do not use "this."" in the case of a mapped object

        /*
        props.onClick({
            friend: friend
            ,id:friend.id      // props can be called back to the specified function "on click"
            ,delete:true
        })
        */
    };

    const onClickEditFriend = function (e) {
        // props.edit(friend.id); // do not use "this."" in the case of a mapped object

        props.onClick({ friend }) // returns value of friend as properties, can also send an object "myFriend:friend"
