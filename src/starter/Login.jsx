...
const logInSuccess = () => {toast.success('🦄 Registration Success!', {
    position: "top-right",
    autoClose: 5000,
...
class LoginPage extends React.Component{
    ...
    <div className="form-group col-2">
        <label>Email</label>
        <input type="email" name="email" className="form-control" placeholder="Enter email" value={this.state.formData.email} onChange={this.onInputLogIn} />
    </div>

    <div className="form-group col-2">
        <label>Password</label>
        <input type="password" name="password" className="form-control" placeholder="Enter password" value={this.state.formData.password} onChange={this.onInputLogIn} />
    </div>
    ...
    <button type="submit" className="btn btn-dark btn-lg btn-block col-2" onClick={this.onSignInClicked} >Sign in</button></div>
    <p className="here forgot-password text-center">
        Forgot <a href="#">password?</a>
    </p>
    <p className="register text-center">
        If you don't have an account <a href="http://localhost:3000/register">Register Here</a>
    </p>
</form>
        )
    }
