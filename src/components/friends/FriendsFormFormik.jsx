...
class FriendsFormFormik extends React.Component {
  ...
  onSave = (values) => {
    ...
    if (values.id) {
      // console.log("if firing");
      updateFriend(values).then(this.onSaveSuccess).catch(this.onSaveError);
    } else {
  ...
  onGetFriendSuccess = (response) => {
    ...
    this.setState((prevState) => {
      ...
      formData.imageId = friend.image.id;
      formData.imageTypeId = friend.image.imageTypeId;
      formData.imageUrl = friend.image.imageUrl;
