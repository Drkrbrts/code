...
class Cars extends React.Component {
  ...
  displayLogic = () => {
    let displayStatus = this.state.showCards;
  ...
  showHideCarsClick = (e) => {
    ...
    // Toggle Vis State On or Off (T/F)
    let display = this.state.showCards
    let toggle = (display = !display)
    this.setState({
      showCards: toggle
    })

    let displayString = this.displayLogic()
    document.querySelector(".car-cards-container").style.display = displayString
  }

  mapCar = (oneCar, index) => {
    return (
      <CardTemplate make={oneCar.make} model={oneCar.model} year={oneCar.year} key={index}></CardTemplate>
    )
  } 
  ...
  formFilter = () => {
    ...
    let year = 2021 || 2020 || 2019
    let filteredYear = this.state.cars.filter(car => car.year.includes(year))
  }
      ...
      <div className="container-fluid">
        <div className="col-md-12 p-5">
          <div className="header-container">
            ...
            <form>
              <select class="form-control" id="form-filter" onChange={this.formFilter}>
                <option></option>
                <option>2021</option>
                <option>2020</option>
                <option>2019</option>
